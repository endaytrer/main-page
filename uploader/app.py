...
def parse_markdown(filename: str, path: str, stat: os.stat_result) -> Blog:
    id = filename
    created: datetime.datetime = datetime.datetime.fromtimestamp(stat.st_ctime)
    last_modified: datetime.datetime = datetime.datetime.fromtimestamp(stat.st_mtime)
    title = None
    license = None
...
def update_hash():
    ...
    for post in posts:
        ...
        elif blog_hashes[post] != datetime.datetime.fromtimestamp(stat.st_mtime):
            # modified post
